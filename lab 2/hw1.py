...
class NewVacuumEnvironment(VacuumEnvironment):
    ...
    def add_random_dirt(self):
        total_cells = (self.width - 1) * (self.height - 1)
        if self.bias == 0:
            return
        elif self.bias == 1:
            for x in range(1, self.width):
                for y in range(1, self.height):
                    if not any(isinstance(thing, Dirt) for thing in self.list_things_at((x, y))):
                        self.add_thing(Dirt(), (x, y))
            return

        else: 
            target_dirt_cells = int(self.bias * total_cells)

            placed_dirt = 0
            while placed_dirt < target_dirt_cells:
    ...
    def get_world(self):
        ...
        for x in range(0, self.width):
            ...
            for y in range(0, self.height):
                ...
                if any(isinstance(thing, Wall) for thing in things):
                    row.append([Wall()])
                elif any(isinstance(thing, Dirt) for thing in things):
                    row.append([Dirt()])
                elif any(isinstance(thing, Agent) for thing in things):
                    row.append([Agent()])
                else:
                    row.append([])
            grid.append(row)
...
def TraceAgent(agent):
    original_program = agent.program
    def traced_program(percept):
        action = original_program(percept)
        print(f"Agent at {agent.location} takes action {action} with performance {agent.performance}")
        return action
    agent.program = traced_program
    return agent
